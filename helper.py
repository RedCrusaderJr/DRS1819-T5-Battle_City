...
class Helper:
    ...
    @staticmethod
    def isBulletCollision(parent_widget, x, y):
        parent_widget.mutex.lock()
        next_position_shape = parent_widget.getShapeType(x, y)  # parent_widget.board[(new_y * gb.GameBoard.BoardWidth) + new_x]
        parent_widget.mutex.unlock()

        if (x >= 0 and y >= 0) and (next_position_shape is ElementType.NONE):
            return False
        return True